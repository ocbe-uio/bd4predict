...
def imputation(data: pd.DataFrame, model) -> pd.DataFrame:
    ...
    cat_features_in = (
        model.named_steps["preprocessing"].named_transformers_["cat"].feature_names_in_
    )
    cat_features_out = (
        model.named_steps["preprocessing"]
    ...
    num_imputed = (
        ...
        .named_transformers_["num"]
        .named_steps["scale"]
        .inverse_transform(processed_data[num_features])
    )
    cat_imputed = (
        model.named_steps["preprocessing"]
        .named_transformers_["cat"]
        .named_steps["onehot"]
        .inverse_transform(processed_data[cat_features_out])
    )
...
class MapieConformalPredictiveDistribution(MapieRegressor):
    ...
    def get_cumulative_distribution_function(self, X, y_pred):
        ...
        "Nonparametric predictive distributions based
                             on conformal prediction" (2017)
        Vovk et al
        ...
        then it can be sorted in increasing order to
    ...
    def predict_proba(self, X, lower=None, upper=None):
        ...
        for observation in range(X.shape[0]):
            ...
            if lower is not None:
                probability[observation] = cdf[self.find_nearest(bins, lower) - 1]
            else:
